...
public class PeriodicTable {
    ...
    public void add (ElementAPI element)
    {
     ...
     */
    public void add (ElementFactoryAPI factory)
    {
    ...
    public<T> void sort(Comparator<? super ElementAPI> c)
    {
//        this.elementList.sort(c);
    	Stream<ElementAPI> elementListStream = elementList.stream();
    	Stream<ElementAPI> sortedElementListStream = (c == null) 
    			? elementListStream.sorted() : elementListStream.sorted(c);
    	elementList = sortedElementListStream.collect(Collectors.toList());    	
    }
    ...
    public String toString() {
    	...
    	stringBuilder.append("Periodic Table\n");
    	this.elementList.stream().forEach(stringBuilder::append);
    ...
    public static class ElementFactory implements ElementFactoryAPI
    {
        ...
        public ElementAPI getObject(String csvString) {
            ...
            try {
                ...
                price = ConvertUtil.convertToDouble(data[1]);
                name = data[2];
                symbol = data[3];
                description = data[4];
            } catch (NumberFormatException e) {
            	System.err.println("Number Format Exception occurred while parsing!");
                e.printStackTrace();
            } catch (Exception e) {
    ...
    public static class ElementLazySingletonFactory  {
        ...
        private static ElementFactoryAPI instance = null; //Lazy singleton
    ...
    public static void demo() {
        ...
        pTable.sort(Comparator.comparing(ElementAPI::getAtomicNumber));
        System.out.println(pTable);

        System.out.println("SORT PeriodicTable elements by Symbol...");
        pTable.sort(Comparator.comparing(ElementAPI::getSymbol));
        System.out.println(pTable);

        System.out.println("SORT PeriodicTable Book elements by NAME...");
        pTable.sort(Comparator.comparing(ElementAPI::getName));
        System.out.println(pTable);

        System.out.println("SORT PeriodicTable Book elements by DESCRIPTION...");
        pTable.sort(Comparator.comparing(ElementAPI::getDescription));
        System.out.println(pTable);
